...
func tarif(jum int, buku *book) {
	...
	fmt.Printf("%6s %40s %20s %10s %10s %10s %10s \n", "ID buku", "Judul", "Tanggal Peminjaman", "Tarif", "Terlambat", "Denda", "Total")
	for i := 0; i < jum; i++ {
		if buku[i].pinjam.pinjam == true {
			fmt.Printf("%6d %40s %20s %10d %10d %10d %10d\n", buku[i].id_book, buku[i].judul, buku[i].pinjam.tanggal.Format("2006-01-02"), buku[i].pinjam.tarif, terlambat, buku[i].pinjam.denda, buku[i].pinjam.tarif+buku[i].pinjam.denda)
		}
	}
	fmt.Scanln()
	menu_utama(jum, *buku)
